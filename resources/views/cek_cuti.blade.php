...
<html>
  <head>
    ...
    <title>Manajemen Cuti BNPB - Cek Sisa Cuti</title>
    ...
    <aside class="max-w-62.5 ease-nav-brand z-990 fixed inset-y-0 my-4 ml-4 block w-full -translate-x-full flex-wrap items-center justify-between overflow-y-auto rounded-2xl border-0 bg-white p-0 antialiased shadow-none transition-transform duration-200 xl:left-0 xl:translate-x-0 xl:bg-transparent">
      <div class="h-19.5">
        ...
        <a class="block px-8 py-6 m-0 text-sm whitespace-nowrap text-slate-700" href="javascript:;" target="_blank">
          ...
          <span class="ml-1 font-semibold transition-all duration-200 ease-nav-brand">Cuti BNPB</span>
        </a>
      </div>
      ...
      <div class="items-center block w-auto max-h-screen overflow-auto h-sidenav grow basis-full">
        <ul class="flex flex-col pl-0 mb-0">
          <li class="mt-0.5 w-full">
            <a class="py-2.7 shadow-soft-xl text-sm ease-nav-brand my-0 mx-4 flex items-center whitespace-nowrap rounded-lg bg-white px-4 font-semibold text-slate-700 transition-colors" href="{{ route('cek-cuti.index') }}">
              ...
              <span class="ml-1 duration-300 opacity-100 pointer-events-none ease-soft">Cek Sisa Cuti</span>
            </a>
          </li>

          <li class="mt-0.5 w-full">
            <a class="py-2.7 text-sm ease-nav-brand my-0 mx-4 flex items-center whitespace-nowrap px-4 transition-colors" href="{{ route('import.index') }}">
              ...
              <span class="ml-1 duration-300 opacity-100 pointer-events-none ease-soft">Import Data</span>
            </a>
          </li>
...
/* Layout Bento Unik */
/* Bento 1 – besar kiri atas */
.bento-1 {
  grid-column: span 4;
  grid-row: span 1;
}

/* Bento 2 – kecil kanan atas */
.bento-2 {
  grid-column: span 2;
  grid-row: span 1;
}

/* Bento 3 – kecil kiri bawah */
.bento-3 {
  grid-column: span 2;
  grid-row: span 1;
}

/* Bento 4 – besar kanan bawah */
.bento-4 {
  grid-column: span 4;
  grid-row: span 1;
}

/* --- Bento-1 lebih tinggi agar form muat --- */
.bento-1 {
  grid-column: span 4;
  grid-row: span 2; /* tambahkan tinggi 2 baris */
}
...
.btn-primary {
  background: #cb0c9f; /* Sesuaikan warna dengan tema Soft UI (ungu/pink) */
  color: white;
}

.btn-export {
  background: #17c1e8; /* Sesuaikan warna dengan tema Soft UI (biru muda) */
  color: white;
}
...
@media (max-width: 768px) {
  ...
  .bento-1 {
    grid-column: span 2;
    grid-row: span 3; /* tambah height pada layar kecil */
  }
}

/* --- Bento-2 diperbesar agar muat semua komponen --- */
.bento-2 {
  grid-column: span 2;
  grid-row: span 2; /* tambah tinggi 2 baris */
  display: flex;
  flex-direction: column;
  gap: 10px;
}
...
/* --- Bento Riwayat: full width bawah --- */
.bento-riwayat, .bento-table {
  grid-column: span 6;   /* penuh */
  grid-row: span 3;      /* tinggi cukup untuk tabel */
  padding: 20px;
  display: flex;
  flex-direction: column;
  gap: 15px;
...
/* Wrapper agar tabel tidak overflow */
.table-wrapper {
  width: 100%;
  overflow-x: auto;       /* aman untuk layar sempit */
}
...
.riwayat-table {
  ...
  font-size: 14px;
  min-width: 800px;       /* mencegah pecah kolom */
}
...
@media (max-width: 768px) {
  .bento-2 {
    grid-column: span 2;
    grid-row: span 3; /* tambah tinggi lagi untuk mobile */
  }
}
...
<div class="bento-container">
  ...
  <div class="bento bento-1">
  <h4>Cek Sisa Cuti</h4>

  @if(session('error'))
    <div style="background-color: #fee2e2; color: #b91c1c; padding: 10px; border-radius: 8px; margin-bottom: 10px; font-size: 14px;">
        {{ session('error') }}
    </div>
  @endif

  <form class="cek-cuti-form" action="{{ route('cek-cuti.check') }}" method="POST">
    @csrf 
    <div class="form-group">
      <label for="keyword">NIP / Nama Pegawai</label>
      <input type="text" id="keyword" name="keyword" placeholder="Masukkan NIP atau Nama" value="{{ old('keyword', $data['pegawai']->nip ?? '') }}" required>
    </div>

    <div class="form-row">
      <div class="form-group">
        <label for="tgl_dari">Tanggal Dari</label>
        <input type="date" id="tgl_dari" name="tgl_dari" value="{{ old('tgl_dari', $data['tahun'] ?? date('Y') . '-01-01') }}">
      </div>
    ...
    <div style="margin-top: 15px; padding: 10px; background-color: #f8fafc; border-radius: 8px;">
        <p style="margin-bottom: 2px; font-size: 13px;"><strong>Nama:</strong> {{ $data['pegawai']->nama }}</p>
        <p style="margin-bottom: 2px; font-size: 13px;"><strong>Jabatan:</strong> {{ $data['pegawai']->jabatan }}</p>
        <p style="margin-bottom: 0; font-size: 13px;"><strong>Unit:</strong> {{ $data['pegawai']->unit_kerja }}</p>
    </div>
    @endif
  ...
  <h4>Sisa Cuti {{ isset($data) ? 'Tahun ' . $data['tahun'] : '' }}</h4>
  
  @if(isset($data))
      <h3 class="sisa-cuti-angka" style="{{ $data['sisa_cuti'] < 0 ? 'color: #ef4444;' : 'color: #82d616;' }}">
          {{ $data['sisa_cuti'] }} hari
      </h3>

      <div class="sisa-detail">
        <p><strong>Jatah dasar:</strong> {{ $data['jatah_dasar'] }} hari</p>
        <p><strong>Carry over tahun lalu:</strong> {{ $data['carry_over_tahun_lalu'] }} hari</p>
        <div style="border-top: 1px solid #ddd; margin: 4px 0;"></div>
        <p><strong>Total jatah:</strong> {{ $data['total_jatah'] }} hari</p>
        <p style="color: #ef4444;"><strong>Cuti diambil:</strong> {{ $data['cuti_terpakai'] }} hari</p>
        <div style="border-top: 1px solid #ddd; margin: 4px 0;"></div>
        <p><strong>Carry over untuk tahun depan:</strong> {{ $data['carry_over_tahun_depan'] }} hari</p>
      </div>
  @else
      <h3 class="sisa-cuti-angka">-</h3>
      <div class="sisa-detail">
        <p class="text-xs text-gray-400">Silakan cari data pegawai terlebih dahulu.</p>
      </div>
  @endif
</div>
  ...
  <div class="bento bento-3">
    <h3>Chart 1 Nanti</h3>
    <p class="text-center text-gray-400" style="padding-top: 20px;">(Grafik Bulanan)</p>
  </div>

  <div class="bento bento-4">
    <h3>Chart 2 nanti</h3>
    <p class="text-center text-gray-400" style="padding-top: 20px;">(Grafik Jenis Cuti)</p>
  </div>

    <div class="bento bento-table" style="grid-column: span 6; overflow-x: auto;">
    <h3>Riwayat Cuti</h3>

    <div class="table-wrapper">
    <table class="riwayat-table" border="1" cellpadding="8" cellspacing="0" width="100%">
      <thead>
        <tr>
          <th>Jenis Cuti</th>
          <th>Tanggal Mulai</th>
      ...
      <tbody>
        @if(isset($data))
            ...
            <tr>
              ...
              <td style="font-weight: bold; text-align: center;">{{ $item->lama_cuti }} hari</td>
              <td>{{ $item->keterangan ?? '-' }}</td>
              <td>
                <span style="background-color: #82d616; color: white; padding: 2px 8px; border-radius: 4px; font-size: 12px;">Disetujui</span>
              </td>
            </tr>
            @empty
            <tr>
              <td colspan="6" style="text-align:center; padding: 20px;">Tidak ada data riwayat cuti.</td>
            </tr>
            @endforelse
        @else
            <tr>
              <td colspan="6" style="text-align:center; padding: 20px;">Data riwayat cuti akan ditampilkan di sini...</td>
            </tr>
        @endif
      </tbody>
    </table>
