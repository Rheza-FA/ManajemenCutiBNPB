...
<style>
    ...
    /* --- CUSTOM CARD STYLE --- */
    .card-custom { background: #ffffff; border: none; border-radius: 16px; box-shadow: 0 4px 20px rgba(0,0,0,0.05); transition: all 0.3s; overflow: hidden; }
    .card-custom:hover { transform: translateY(-5px); box-shadow: 0 12px 30px rgba(0,0,0,0.08); }
    .card-header-custom { background: #f8fafc; border-bottom: 1px solid #e2e8f0; padding: 18px 24px; font-weight: 700; color: #0f3878; display: flex; align-items: center; gap: 10px; }

    /* --- FORM & BUTTONS --- */
    .form-control-custom { border: 1px solid #cbd5e1; padding: 12px 16px; border-radius: 10px; transition: 0.2s; }
    .form-control-custom:focus { border-color: #ff6b00; box-shadow: 0 0 0 4px rgba(255, 107, 0, 0.1); outline: none; }
    
    .btn-bnpb-primary { background: linear-gradient(135deg, #ff6b00 0%, #ea580c 100%); border: none; color: white; padding: 10px 28px; border-radius: 10px; transition: 0.3s; }
    .btn-bnpb-primary:hover { transform: translateY(-1px); }
    .btn-bnpb-secondary { background: #e0f2fe; color: #0ea5e9; border: none; padding: 10px 24px; border-radius: 10px; transition: 0.3s; }
    .btn-bnpb-secondary:hover { background: #bae6fd; color: #0284c7; }

    /* --- HERO & STATS --- */
    .card-hero { background: linear-gradient(135deg, #0f3878 0%, #1e4d9c 100%); color: white; border: none; border-radius: 16px; overflow: hidden; transition: 0.3s; }
    .card-hero:hover { transform: translateY(-5px); box-shadow: 0 15px 35px rgba(15, 56, 120, 0.4) !important; }
    .stat-box-light { background: rgba(255, 255, 255, 0.1); border-radius: 8px; padding: 10px; backdrop-filter: blur(5px); height: 100%; }
    
    .nav-pills-custom .nav-link { color: #64748b; background: #fff; border: 1px solid #e2e8f0; border-radius: 8px; font-weight: 600; padding: 10px 20px; margin-right: 10px; transition: 0.2s; }
    .nav-pills-custom .nav-link.active { background-color: var(--bnpb-blue); color: #fff; border-color: var(--bnpb-blue); }
    
    @media (max-width: 767.98px) {
        .btn-mobile-fix { padding: 8px 10px !important; font-size: 0.85rem !important; width: 100%; display: flex !important; justify-content: center; }
    }
</style>

<div class="d-flex justify-content-between align-items-center mb-5 fade-in-up">
    <div>
        <h3 class="fw-bold text-bnpb-blue mb-1 d-flex align-items-center">
            <i class="bi bi-speedometer2 me-2"></i> Dashboard Cuti
        </h3>
        <p class="text-muted mb-0 small ms-1">Pantau status cuti dan riwayat pengajuan pegawai.</p>
    </div>
    <div class="d-none d-md-flex align-items-center bg-white px-4 py-2 rounded-pill shadow-sm border">
        <div class="me-3 text-end lh-1">
            ...
            <span class="d-block text-muted" style="font-size: 10px;">PEGAWAI ASN</span>
        </div>
        <div class="rounded-circle bg-light d-flex align-items-center justify-content-center text-bnpb-orange fw-bold shadow-sm" style="width: 40px; height: 40px; font-size: 1.1rem;">A</div>
    </div>
</div>
...
<div class="row g-4">
    ...
    <div class="col-lg-8 fade-in-up delay-100">
        <div class="card card-custom h-100">
            <div class="card-header-custom">
                <i class="bi bi-search text-bnpb-orange"></i> <span>Pencarian Data Pegawai</span>
            </div>
                ...
                <form id="searchForm" action="{{ route('cek-cuti.check') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="input-group">
                            <span class="input-group-text bg-white border-end-0 rounded-start-3" style="border-color: #cbd5e1;"><i class="bi bi-person text-muted"></i></span>
                            <input type="text" class="form-control form-control-custom border-start-0 rounded-end-3" id="keyword" name="keyword" placeholder="Masukkan NIP atau Nama Lengkap..." value="{{ old('keyword', $data['pegawai']->nip ?? '') }}" required>
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            ...
                            <input type="date" class="form-control form-control-custom" id="tgl_dari" name="tgl_dari" value="{{ old('tgl_dari', $data['tahun'] ?? date('Y') . '-01-01') }}">
                        </div>
                        <div class="col-md-6">
                    ...
                    <div class="d-flex gap-2 gap-md-3">
                        <button type="submit" class="btn btn-bnpb-primary btn-mobile-fix flex-grow-1 flex-md-grow-0" id="btnCari">
                            <i class="bi bi-search me-2"></i> <span>Cari Data</span>
                        </button>
                        ...
                        <button type="submit" formaction="{{ route('cek-cuti.export') }}" class="btn btn-bnpb-secondary btn-mobile-fix flex-grow-1 flex-md-grow-0">
                            <i class="bi bi-file-earmark-excel me-2"></i> <span>Export Excel</span>
                        </button>
                        @endif
                    </div>
                </form>
                ...
                <div class="mt-4 pt-4 border-top">
                    <div class="d-flex align-items-start">
                        <div class="flex-shrink-0">
                             <div class="rounded-3 bg-light p-3 text-bnpb-blue shadow-sm"><i class="bi bi-person-badge fs-3"></i></div>
                        </div>
        ...
        @else
            <div class="card card-custom text-center py-5 h-100 justify-content-center">
                <div class="card-body">
                    ...
                    <p class="small text-muted mb-0">Lakukan pencarian pegawai di kiri.</p>
                </div>
            </div>
        @endif
    </div>
    ...
    <div class="col-12 fade-in-up delay-300">
        ...
        <ul class="nav nav-pills nav-pills-custom mb-3" id="pills-tab" role="tablist">
            <li class="nav-item">
                <button class="nav-link active" id="pills-history-tab" data-bs-toggle="pill" data-bs-target="#pills-history">
        ...
        <div class="tab-content" id="pills-tabContent">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="px-4 py-3">
                                            <span class="fw-bold text-bnpb-blue small">
                                                @if(stripos($item->jenis_cuti, 'alasan penting') !== false)
                                                    <span class="d-md-none">Cuti CAP</span><span class="d-none d-md-inline">{{ $item->jenis_cuti }}</span>
                                                @else
                                                    {{ $item->jenis_cuti }}
                                                @endif
                                            </span>
                                        ...
                                        <td class="px-4 py-3 text-center">
                                            ...
                                            @endphp
                                            @if($bisaCetak)
                                                <a href="{{ route('cetak.surat', ['id' => $item->id]) }}" class="btn btn-sm btn-outline-success mb-0 shadow-none px-3" title="Download Surat Word"><i class="bi bi-file-earmark-word-fill me-1"></i> Word</a>
                                            @else
                                                <span class="text-muted small">-</span>
                                            @endif
                                        </td>
                                        <td class="px-4 py-3">
                                            <span class="badge bg-success bg-opacity-10 text-success badge-status border border-success border-opacity-25"><i class="bi bi-check-circle-fill me-1"></i> Disetujui</span>
                                        </td>
                                    </tr>
                                    @empty
                                    <tr>
            ...
            <div class="tab-pane fade" id="pills-stats">
                <div class="row g-4">
                    <div class="col-md-6">
                        <div class="card card-custom h-100">
                            <div class="card-header-custom"><i class="bi bi-pie-chart text-bnpb-orange"></i> Statistik Bulanan</div>
                            <div class="card-body p-3"><div style="height: 300px; position: relative;"><canvas id="chartDonut"></canvas></div></div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="card card-custom h-100">
                            <div class="card-header-custom"><i class="bi bi-graph-up text-bnpb-orange"></i> Tren Jenis Cuti</div>
                            <div class="card-body p-3"><div style="height: 300px; position: relative;"><canvas id="chartLine"></canvas></div></div>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    if(form) {
        form.addEventListener('submit', function() {
            btnCari.innerHTML = '<span class="spinner-border spinner-border-sm me-2"></span> Memproses...';
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        new Chart(ctxDonut, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Hari Cuti', data: donutData,
                    backgroundColor: ['#0f3878', '#ff6b00', '#3b82f6', '#10b981', '#f59e0b', '#ef4444', '#6366f1', '#8b5cf6', '#ec4899', '#14b8a6', '#84cc16', '#64748b'],
                    borderWidth: 2, borderColor: '#ffffff', hoverOffset: 6
                }]
            },
            options: { responsive: true, maintainAspectRatio: false, cutout: '70%', plugins: { legend: { position: 'right', labels: { boxWidth: 12, usePointStyle: true, font: { size: 11 } } } }, animation: { animateScale: true, animateRotate: true, duration: 1500, easing: 'easeOutQuart' } }
        });

        const ctxLine = document.getElementById('chartLine').getContext('2d');
        new Chart(ctxLine, {
            type: 'line',
            data: { labels: ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun', 'Jul', 'Agu', 'Sep', 'Okt', 'Nov', 'Des'], datasets: lineDataSets },
            options: { responsive: true, maintainAspectRatio: false, interaction: { mode: 'index', intersect: false }, plugins: { legend: { position: 'bottom', labels: { usePointStyle: true, padding: 20 } }, tooltip: { enabled: true, backgroundColor: 'rgba(0, 0, 0, 0.8)', padding: 12, titleFont: { size: 14 }, bodyFont: { size: 13 } } }, scales: { y: { beginAtZero: true, ticks: { stepSize: 1, font: { size: 11 } }, grid: { borderDash: [5, 5] } }, x: { grid: { display: false }, ticks: { font: { size: 11 } } } }, elements: { line: { tension: 0.4 }, point: { radius: 4, hitRadius: 20, hoverRadius: 6 } }, animation: { duration: 2000, easing: 'easeOutQuart' } }   
        });
    });
</script>
@endif
